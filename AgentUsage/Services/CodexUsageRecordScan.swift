...
/// one is the session total; the newest `token_count` total is the fallback.
///
...
nonisolated struct CodexUsageRecordScan {
    /// Newest `token_count` `info.total_token_usage`.
    ...
    func sessionTotal(
        ...
        let tokenCount = tokenCountTotal.map { total in
            inheritedTokenCount.map { total.excluding(inherited: $0) } ?? total
        }
    ...
    /// Feeds one `token_count` event's `info.total_token_usage`. Lines must arrive newest first.
    mutating func recordTokenCount(_ total: CodexCumulativeTokenUsage) {
